    ...
    for url in url_list:
        ...
        if len(data) > 0:
            ...
            with open("data" + os.sep + file_name + "." + run_time + ".csv", "w") as f:
                ...
                for row in data:
                    f.write(
                        config.DELIM.join((file_name, url, run_time))
                        + config.DELIM
                        + config.DELIM.join([str(x or "") for x in row])
                        + "\n"
                    )
...
def get_html(u, file_name):
    ...
    if False and os.path.isfile(file_name):
        ...
        service = Service(executable_path=config.CHROMEDRIVER_BINARY_LOCATION)
        ...
        # get page
        d.get(u)
        # WebDriverWait(d, config.SLEEP_SEC)
        d.find_element(
            By.XPATH,
            "/html/body/c-wiz/div/div/div/div[2]/div[1]/div[4]/div[1]/div[1]/form[2]/div/div/button",
        ).click()
        ...
        try:
            WebDriverWait(d, config.SLEEP_SEC).until(
                EC.presence_of_element_located(
                    # (By.CLASS_NAME, "section-popular-times-bar")
                    (By.CLASS_NAME, "dpoVLd")
                )
            )
        except TimeoutException:
            print(
                'ERROR: Timeout! (This could be due to missing "popular times" data, or not enough waiting.)',
                u,
            )
...
def parse_html(html):
    ...
    for pop in pops:
        ...
        if t.startswith("Pieno"):
            hour = int(t.split()[5][:-4])
            freq = int(t.split()[2][:-1])  # gm uses int
        else:
            # the current hour has special text
            # hour is the previous value + 1
            hour = hour + 1
            freq_now = int(t.split()[3][:-2])
            freq = int(t.split()[8][:-2])
